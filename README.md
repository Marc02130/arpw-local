...
Implementation has not started. Spec: [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md). Slice order: [`docs/PLAN.md`](docs/PLAN.md).
...
| Tutorial | [Get to the dashboard](#tutorial-get-to-the-dashboard) (after PR 01) |
| How-to | Written as slices land (confirm email, upload, Grok key, generate, tests) |
| Reference | [Ports and env](#ports-and-env), [routes](#routes) (target; not running yet) |
...
Not runnable until [PR 01](docs/PLAN.md#pr-01--compose-scaffold) lands. Target flow:

### What you'll need

- Docker
- An xAI Grok API key only when you generate, outline, or interrogate (uploads embed on-server with MiniLM)
...
A confirmed user, an `arpw_session` cookie, and access to `/dashboard`, `/generate`, `/profile`, and `/library`. Next: upload references, paste OpenAI / xAI / Anthropic keys on Profile, interrogate, generate.
...
| `/generate` | Prompt tab: research prompt, **section checkboxes**, **Paper Type / Citation Style / Output Format dropdowns**, generate |
| `/generate/upload` | Literature, original research, examples |
| `/generate/interrogate` | Grounded Q&A; pin passages |
...
Literature-review UAT waits are **not** ARPW’s 180s/300s. See [ARCHITECTURE.md](docs/ARCHITECTURE.md) Testing and [PLAN.md](docs/PLAN.md) PR 11.
